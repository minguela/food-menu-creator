...
const baseUrl = (process.argv.find((arg) => arg.startsWith('--base-url=')) || process.argv[baseUrlArgIndex + 1] || '')
  .split('=')[1]
...
async function main() {
  ...
  for (const route of routes) {
    ...
    if (!matched) {
      failures.push(`${route.path}: missing expected content (${route.expected.join(', ')})`)
    }
  }
