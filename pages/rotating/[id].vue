<template>
  <div class="space-y-6">
    ...
    >
      ...
      <pre
        v-if="debug"
        class="mt-3 max-h-80 overflow-auto rounded border border-red-200 bg-white p-3 text-xs"
        >{{ JSON.stringify(debug, null, 2) }}</pre>
    </section>

    <section
      v-else-if="!detail || detail.days.length === 0"
    ...
    <template v-else>
      ...
      <section class="rounded-lg border bg-white p-4 text-gray-900">
        ...
        <div class="mt-3 flex gap-2 overflow-x-auto pb-1">
          <button
            ...
            :key="`nav-${day.id}`"
            class="min-w-16 rounded border px-3 py-2 text-xs"
            :class="
      ...
      <section v-if="showDebug" class="rounded-lg border bg-zinc-950 p-4 text-xs text-zinc-100">
        ...
        <pre class="max-h-96 overflow-auto rounded border border-zinc-800 p-3">{{ JSON.stringify(detail.debug, null, 2) }}</pre>
      </section>
                ...
                <table class="min-w-[900px] w-full text-xs">
                  ...
                  <tbody>
                    ...
                    >
                      ...
                      <td class="px-2 py-2">
                        <div
                          v-if="portion.ingredients.length > 0"
      ...
      <section class="rounded-lg border bg-white p-4 text-gray-900">
        ...
        <div
          v-if="detail.shopping_items.length === 0"
        ...
        <div v-else class="mt-3 grid gap-2 md:grid-cols-2">
          <div
            v-for="item in detail.shopping_items"
            :key="item.id"
