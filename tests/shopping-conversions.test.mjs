...
test('converts weight units to grams exactly', () => {
  assert.deepEqual(convertToGrams({ name: 'arroz', quantity: 1.5, unitType: 'kg' }), {
    grams: 1500,
    status: 'exact',
...
test('converts liquid units with known density', () => {
  const result = convertToGrams({ name: 'aceite de oliva', quantity: 100, unitType: 'ml' })
...
test('marks unknown pieces as ambiguous and editable', () => {
  const result = convertToGrams({ name: 'servilletas', quantity: 2, unitType: 'pack' })
...
test('exports shopping list as plain text and csv', () => {
  const items = [
    { item_name: 'papel higiénico', quantity_grams: 500, purchased: false, conversion_status: 'manual' },
    { ingredients: { name: 'arroz' }, quantity_grams: 1000, purchased: true, conversion_status: 'exact' },
  ]
