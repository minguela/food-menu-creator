  ...
  <div>
    <div class="flex flex-wrap justify-between items-center gap-3 mb-6">
      <div>
        ...
        <p class="text-sm text-gray-500">Cantidades normalizadas a gramos, con edición manual cuando la conversión es dudosa.</p>
      </div>
      <button
    ...
    <div v-else class="space-y-6">
      <section class="grid gap-4 md:grid-cols-4">
        ...
        <div class="bg-white rounded-lg shadow-sm border p-4">
          ...
          <p class="text-2xl font-bold" :class="ambiguousCount ? 'text-amber-700' : 'text-gray-900'">{{ ambiguousCount }}</p>
        </div>
        <div class="bg-white rounded-lg shadow-sm border p-4">
          <p class="text-sm text-gray-600">Estado móvil</p>
          <p class="text-lg font-semibold text-gray-900">{{ sendStatusLabel }}</p>
        </div>
      </section>
      ...
      <section class="bg-white rounded-lg shadow-sm border p-4">
        ...
        <form class="grid gap-2 md:grid-cols-[1fr_130px_auto]" @submit.prevent="addExtraItem">
          ...
          <input
            ...
            required
          />
          <button class="bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700">Añadir</button>
        </form>
      </section>
      ...
      <section class="bg-white rounded-lg shadow-sm border p-4">
        <div class="flex flex-wrap items-end gap-3">
          <label class="flex-1 min-w-[220px]">
            <span class="block text-sm font-medium text-gray-700 mb-1">Número de teléfono</span>
            <input v-model.trim="phoneNumber" class="w-full border rounded-lg px-3 py-2" placeholder="+34600111222" />
          </label>
          <label>
            <span class="block text-sm font-medium text-gray-700 mb-1">Canal</span>
        ...
        <p v-if="sendMessage" class="text-sm mt-3" :class="sendError ? 'text-red-600' : 'text-emerald-700'">{{ sendMessage }}</p>
      </section>
        ...
        <div class="divide-y">
          ...
          >
            <div class="flex items-start gap-3">
              ...
              <div :class="{ 'line-through text-gray-400': item.purchased }">
                <div class="flex flex-wrap items-center gap-2">
                  <p class="font-medium text-gray-900">{{ item.item_name || item.ingredients?.name || 'Artículo' }}</p>
                  <span
                ...
                <p class="text-sm text-gray-500">{{ item.conversion_note || originalQuantity(item) }}</p>
              </div>
            </div>
            <label>
              <span class="sr-only">Cantidad en gramos</span>
              <input
                :value="Math.round(Number(item.quantity_grams || item.quantity_needed || 0))"
                type="number"
                min="1"
                step="1"
            ...
            <div class="text-right">
              ...
              <p class="text-sm text-gray-500">{{ Math.round(Number(item.quantity_grams || item.quantity_needed || 0)) }} g</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <div class="flex flex-wrap justify-end gap-2 pt-4">
        <button @click="markAllAsPurchased" class="px-4 py-2 text-gray-700 hover:bg-gray-100 rounded-lg">
...
import { buildShoppingCsv, convertToGrams } from '~/utils/shopping-conversions.js'
...
const itemsByCategory = computed(() => {
  return items.value.reduce((acc, item) => {
    const category = item.is_extra ? 'Añadidos por ti' : item.ingredients?.carrefour_category || 'Otros'
...
const totalPrice = computed(() => items.value.reduce((sum, item) => sum + (item.estimated_price || 0), 0))
const ambiguousCount = computed(() => items.value.filter((item) => item.conversion_status === 'ambiguous').length)
const sendStatusLabel = computed(() => {
  const status = items.value.find((item) => item.send_status && item.send_status !== 'pending')?.send_status || 'pending'
...
const ensureGramFields = async (list: ShoppingListItem[]) => {
  ...
  for (const item of list) {
    ...
    const payload = {
      ...
      original_unit_type: item.original_unit_type || item.ingredients?.unit_type || 'g',
...
const markAllAsPurchased = async () => {
  const ids = items.value.filter((item) => !item.purchased).map((item) => item.id)
  if (ids.length === 0) return

  const { error } = await supabase.from('shopping_lists').update({ purchased: true }).in('id', ids)
  if (error) return console.error('Error marcando todos:', error)
  items.value.forEach((item) => { item.purchased = true })
}
...
const sendToMobile = async () => {
  ...
  try {
    const result = await $fetch<{ status: string; providerStatus: string }>('/api/send-shopping-list', {
      method: 'POST',
      body: { userId: currentUser.id, phoneNumber: phoneNumber.value, channel: mobileChannel.value },
    })
    sendMessage.value = result.status === 'delivered'
...
const downloadCsv = () => {
  const blob = new Blob([buildShoppingCsv(items.value)], { type: 'text/csv;charset=utf-8' })
...
<style scoped>
@media print {
  nav, button, form, select, input[type='checkbox'] {
