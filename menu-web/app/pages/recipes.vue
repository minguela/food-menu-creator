<template>
  <div class="min-h-screen bg-transparent">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <!-- Search & Filters -->
      <section class="ui-surface rounded-2xl border border-slate-100 border-slate-800 shadow-sm p-5 mb-6">
          ...
          <label class="flex-1 min-w-[240px]">
            <div class="relative">
              ...
              <input v-model.trim=" searchTerm "
                class="w-full pl-10 pr-4 py-2.5 border ui-divider rounded-xl focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all"
                placeholder="Buscar receta por nombre..." />
            </div>
          </label>
          <label class="inline-flex items-center gap-2 text-sm ui-muted ui-muted cursor-pointer hover:text-white ui-title">
            <input type="checkbox" :checked=" allFilteredSelected " @change=" toggleSelectAllFiltered "
              class="w-4 h-4 rounded border-[var(--color-border-strong)] border-[var(--color-border-strong)] text-purple-600 focus:ring-purple-500" />
        ...
        <div v-if=" showMergePanel " class="mt-3 border rounded-lg p-3 space-y-2">
          <p class="text-sm font-medium ui-title ui-title">Fusionar recetas</p>
          <label class="block">
            <span class="block text-xs ui-subtle ui-muted mb-1">Receta destino</span>
          ...
          <label class="block">
            <span class="block text-xs ui-subtle ui-muted mb-1">
      ...
      <section class="space-y-3">
        <article v-if=" filteredDishes.length === 0 " class="ui-surface rounded-lg border p-5 text-sm ui-subtle ui-subtle">
        ...
        <article v-for=" dish in filteredDishes " :key=" dish.id " class="ui-surface rounded-lg border p-4">
            ...
            <div class="flex items-start gap-3">
              ...
              <div>
                <div class="flex items-center gap-2">
                  <h2 class="font-semibold ui-title ui-title">{{ dish.name }}</h2>
                  <span class="rounded-full bg-[var(--color-surface-3)] bg-[var(--color-surface-3)] px-2 py-0.5 text-[11px] font-medium ui-muted ui-muted">
                    {{ ingredientCount( dish ) }} ingredientes
                  </span>
                  <span
                    v-if="recipeBlockersCount( dish ) > 0"
                    class="rounded-full bg-red-100 bg-red-900/40 px-2 py-0.5 text-[11px] font-medium text-red-700 text-red-300"
                  >
          ...
          <div v-if=" editingDishId === dish.id " class="mt-4 space-y-3">
            <div class="rounded-lg border p-3 space-y-2">
              <p class="text-xs font-medium ui-muted ui-muted">Datos de receta</p>
              <div class="grid gap-2 md:grid-cols-2">
                <label>
                  <span class="block text-xs ui-subtle ui-muted mb-1">Nombre</span>
                  <input v-model.trim=" recipeForm.name " class="w-full border rounded-lg px-3 py-2" />
                </label>
                <label>
                  <span class="block text-xs ui-subtle ui-muted mb-1">Descripción</span>
                  <input v-model.trim=" recipeForm.description " class="w-full border rounded-lg px-3 py-2" />
                </label>
                <label class="md:col-span-2">
                  <span class="inline-flex items-center gap-2 text-xs ui-muted ui-muted">
            ...
            <div
              ...
              class="rounded-lg border border-red-200 border-red-900/60 bg-red-50 bg-red-950/30 p-3 space-y-2"
            >
              <p class="text-xs font-semibold text-red-800 text-red-300">
                Ingredientes que bloquean la generación
              </p>
              <ul class="space-y-1 text-xs text-red-700 text-red-300">
                <li
                ...
                >
                  ...
                  <button
                    ...
                    class="rounded border border-red-300 border-red-700 px-2 py-0.5 text-[11px] font-medium hover:bg-red-100/60 hover:bg-red-900/50"
              ...
              <p class="text-[11px] text-red-700/90 text-red-300/90">
                Solución: vincula ingrediente de catálogo o completa nutrición en Ingredientes.
              </p>
            </div>

            <h3 class="text-sm font-medium ui-title ui-title">
            ...
            <div v-if=" pendingRows.length === 0 " class="text-sm ui-subtle ui-subtle">
            ...
            <div class="space-y-2 rounded-xl border ui-divider bg-[var(--color-surface-3)] bg-[var(--color-surface-2)]/60 p-3">
            ...
            <button
              class="inline-flex items-center gap-2 rounded-lg border border-indigo-200 ui-surface px-3 py-1.5 text-sm font-medium text-indigo-700 hover:bg-indigo-50"
            ...
            <div class="rounded-lg border border-sky-100 bg-sky-50/60 p-3 space-y-2">
              ...
              <div class="flex flex-wrap gap-2">
                ...
                <button
                  class="text-xs px-3 py-2 rounded border border-sky-200 ui-surface text-sky-800 hover:bg-sky-100"
              ...
              <p
                ...
                class="text-xs text-slate-500 ui-subtle">
            ...
            <div class="rounded-lg border p-3 space-y-2">
              <p class="text-xs font-medium ui-muted ui-muted">
...
aceite de oliva" />
      ...
      <div v-if=" showSplitPanel " class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4"
        ...
        <div class="w-full max-w-2xl max-h-[90vh] overflow-hidden rounded-lg ui-surface flex flex-col">
          <div class="p-4 space-y-3 overflow-y-auto">
          <h3 class="text-lg font-semibold ui-title ui-title">Dividir receta</h3>
          <p class="text-sm ui-subtle ui-muted">
            Receta original:
          ...
          <div v-else class="space-y-2">
            <p class="text-xs ui-subtle ui-muted">
          ...
          <div class="flex justify-end gap-2 p-4 border-t ui-surface shrink-0">
      ...
      <div v-if=" showCreateRecipeModal " class="fixed inset-0 z-50 flex items-center justify-center p-4"
        ...
        <div class="relative w-full max-w-2xl max-h-[90vh] overflow-hidden rounded-lg ui-surface flex flex-col">
          <div class="p-4 space-y-3 overflow-y-auto">
          <h3 class="text-lg font-semibold ui-title ui-title">Crear receta nueva</h3>
          <label class="block">
            <span class="block text-xs ui-subtle ui-muted mb-1">Nombre</span>
          ...
          <label class="block">
            <span class="block text-xs ui-subtle ui-muted mb-1">Ingredientes (uno por línea)</span>
...
const filter = ref<"all" | "suggested" | "complete" | "not_required" | "incomplete_nutrition">( "all" );
const filterItems = [
...
const statusMeta = ( dish: DishRow ) => {
  ...
  if ( status === "not_required" )
    return { label: "No requiere ingredientes", color: "ui-subtle ui-subtle" };
...
const filteredDishes = computed( () =>
  dishes.value.filter( ( dish ) => {
    ...
    if ( filter.value === "incomplete_nutrition" ) return dish.recipe_status === "incomplete_nutrition";
