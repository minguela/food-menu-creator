<template>
  <div class="space-y-6">
    <header class="flex flex-wrap justify-between gap-3 items-end">
      <div>
        <h1 class="text-2xl font-bold text-gray-900">Histórico de menús mensuales</h1>
        <p class="text-sm text-gray-500">Busca, revisa, reutiliza y exporta menús ya generados.</p>
      </div>
      <input
        v-model.trim="search"
    ...
    <div v-else-if="pagedMenus.length === 0" class="text-center py-12 bg-white rounded-lg border">
        ...
        <div class="flex flex-wrap justify-between gap-3">
          <div>
            ...
            <p class="text-sm text-gray-500">
              {{ formatDate(menu.start_date) }} - {{ formatDate(menu.end_date) }} · {{ menu.menu_data?.length || 0 }} días
            </p>
          </div>
          <div class="flex flex-wrap gap-2">
            <button @click="selected = menu" class="px-3 py-2 text-gray-700 hover:bg-gray-100 rounded-lg">Ver</button>
            <button @click="reuseMenu(menu)" class="px-3 py-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700">Volver a usar</button>
            <button @click="downloadCsv(menu)" class="px-3 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700">CSV</button>
            <button @click="printMenu(menu)" class="px-3 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-700">PDF</button>
          </div>
        </div>
      </article>
    </section>

    <footer v-if="totalPages > 1" class="flex justify-center items-center gap-3">
      <button :disabled="page === 1" @click="page--" class="px-3 py-2 rounded-lg hover:bg-gray-100 disabled:opacity-40">Anterior</button>
      <span class="text-sm text-gray-600">Página {{ page }} de {{ totalPages }}</span>
      <button :disabled="page === totalPages" @click="page++" class="px-3 py-2 rounded-lg hover:bg-gray-100 disabled:opacity-40">Siguiente</button>
    </footer>
    ...
    >
      <div class="bg-white rounded-lg max-w-4xl mx-auto p-6">
        <div class="flex justify-between gap-3 mb-4">
          ...
          <button @click="selected = null" class="text-gray-500 hover:text-gray-800">Cerrar</button>
        </div>
        <div class="grid gap-3 md:grid-cols-2">
          <div v-for="day in selected.menu_data" :key="day.day" class="border rounded-lg p-3">
            <p class="font-medium text-gray-900">Día {{ day.day }} · {{ formatDate(day.date) }}</p>
...
const filteredMenus = computed(() => {
  ...
  return menus.value.filter((menu) => {
    const haystack = [
      menu.name,
      ...(menu.menu_data || []).flatMap((day: any) => [day.desayuno, day.comida, day.cena, day.menu_name]),
    ].join(' ').toLowerCase()
...
const pagedMenus = computed(() => filteredMenus.value.slice((page.value - 1) * pageSize, page.value * pageSize))
...
const reuseMenu = async (menu: MonthlyMenu) => {
  ...
  await supabase.from('shopping_lists').delete().eq('user_id', currentUser.id).eq('week_start', today.toISOString().split('T')[0])
...
const downloadCsv = (menu: MonthlyMenu) => {
  const blob = new Blob([buildShoppingCsv(menu.shopping_list || [])], { type: 'text/csv;charset=utf-8' })
