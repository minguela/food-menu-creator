  ...
  <div class="space-y-6">
    <header class="ui-surface rounded-2xl p-5">
      <div class="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h1 class="mt-1 text-2xl font-semibold text-white">Expansiones</h1>
          <p class="mt-1 text-sm ui-muted">Reglas para anadir ingredientes automaticamente en platos OCR.</p>
        </div>
        <div class="flex items-center gap-2">
          <NuxtLink href="/ingredients" class="rounded-2xl border border-white/25 px-4 py-2 text-sm text-white hover:bg-[var(--color-surface-3)]/60">
            Volver a ingredientes
          </NuxtLink>
          <button class="rounded-2xl border border-white/40 bg-[var(--color-surface-3)]/60 px-4 py-2 text-sm font-medium text-white hover:bg-[var(--color-surface-3)]/80" @click="openExpansionModal()">
            Nueva expansion
          </button>
        </div>
      </div>
    ...
    <section class="ui-surface rounded-2xl p-5">
      ...
      <div v-else-if="expansionMappings.length === 0" class="py-12 text-center">
        <p class="text-base text-white">No hay expansiones definidas</p>
      ...
      <div v-else class="grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
        <article v-for="m in expansionMappings" :key="m.id" class="ui-card rounded-xl p-4">
          <div class="flex items-start justify-between gap-2">
            <div class="min-w-0">
              <div class="flex items-center gap-2">
                <h2 class="truncate text-base font-semibold text-white">{{ m.dish_name }}</h2>
                <span v-if="m.is_global" class="rounded-full border border-white/30 px-2 py-0.5 text-[11px] text-white/90">Global</span>
              </div>
              <p class="mt-1 truncate text-xs ui-subtle">{{ m.aliases?.length ? m.aliases.join(', ') : 'Sin alias' }}</p>
            </div>
            <div class="flex items-center gap-1">
              <button class="rounded-lg border border-white/20 px-2 py-1 text-xs text-white hover:bg-[var(--color-surface-3)]/60" @click="openExpansionModal(m)">Editar</button>
          ...
          <div class="mt-3 flex flex-wrap gap-1.5">
            <span v-for="(ing, i) in (m.ingredients || []).slice(0, 5)" :key="i" class="rounded-md border border-white/20 px-2 py-1 text-xs text-white/90">
              {{ ing.name }}
            </span>
    ...
    <div v-if="showExpansionModal" class="fixed inset-0 z-50 flex items-center justify-center p-4" @click.self="showExpansionModal = false">
      ...
      <div class="relative w-full max-w-xl rounded-2xl border border-white/15 bg-black p-5">
        <h2 class="text-lg font-semibold text-white">{{ editingExpansion ? "Editar" : "Nueva" }} expansion</h2>
