<template>
  <div class="space-y-6">
    <header class="flex flex-wrap items-start justify-between gap-3">
      <div>
        <NuxtLink href="/history" class="text-sm text-accent underline">
          Volver a rotativos
        </NuxtLink>
        <h1 class="mt-2 text-2xl font-bold text-text-1">
      ...
      <div class="flex flex-wrap gap-2">
        <NuxtLink href="/shopping" class="rounded-lg border px-4 py-2 text-sm text-text-2 hover:bg-white/6 bg-surface-1">
          Abrir compra
        </NuxtLink>
        <button class="rounded-lg border px-4 py-2 text-sm text-text-2 hover:bg-white/6 bg-surface-1"
...
=======
    <section v-if=" loading " class="rounded-lg border bg-surface-1 p-6 text-sm text-text-3">
>>>>>>> Stashed changes
      Cargando menú generado...
    </section>

    <section v-else-if=" error " class="rounded-lg border border-danger/20 bg-danger/6 p-5 text-sm ">
...
=======
        class="mt-3 max-h-80 overflow-auto rounded border border-danger/20 bg-surface-1 p-3 text-xs">{{ JSON.stringify( debug, null, 2 ) }}</pre>
>>>>>>> Stashed changes
    </section>

    <section v-else-if=" !detail || detail.days.length === 0 "
      class="rounded-lg border border-warning/20 bg-warning/8 p-5 text-sm text-warning">
...
<<<<<<< Updated upstream
        ...
        <article v-if=" detail.menu?.generator_type === 'nutrition_scored' " class="rounded-lg border bg-[var(--surface-1)] p-4 text-[var(--text-1)]">
          ...
          <p class="mt-1 font-semibold text-[var(--text-1)]">
=======
        <article class="rounded-lg border bg-surface-1 p-4 text-text-1">
          <p class="text-xs text-text-3">Estado job</p>
          <p class="mt-1 font-semibold text-text-1">
            {{ detail.job?.status || "Sin job vinculado" }}
          </p>
        </article>
        <article class="rounded-lg border bg-surface-1 p-4 text-text-1">
          <p class="text-xs text-text-3">Días</p>
          <p class="mt-1 font-semibold text-text-1">{{ detail.days.length }}</p>
        </article>
        <article class="rounded-lg border bg-surface-1 p-4 text-text-1">
          <p class="text-xs text-text-3">Comidas</p>
          <p class="mt-1 font-semibold text-text-1">{{ mealsCount }}</p>
        </article>
        <article class="rounded-lg border bg-surface-1 p-4 text-text-1">
          <p class="text-xs text-text-3">Compra</p>
          <p class="mt-1 font-semibold text-text-1">
            {{ detail.shopping_items.length }} líneas
          </p>
        </article>
        <article v-if=" detail.menu?.generator_type === 'nutrition_scored' " class="rounded-lg border bg-surface-1 p-4 text-text-1">
          <p class="text-xs text-text-3">Score</p>
          <p class="mt-1 font-semibold text-text-1">
>>>>>>> Stashed changes
...
=======
      <section class="rounded-lg border bg-surface-1 p-4 text-text-1">
>>>>>>> Stashed changes
        <div class="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 class="font-semibold text-text-1">Semanas</h2>
...
=======
            :class="selectedWeekNumber === week.weekNumber ? 'border-indigo-500 text-text-1'
                : ' text-text-2 hover:bg-white/6 hover:bg-white/6'
>>>>>>> Stashed changes
      ...
      <section v-if=" showDebug " class="rounded-lg border bg-shell p-4 text-xs text-text-1">
        <h2 class="mb-2 font-semibold">Debug recuperación/render</h2>
        <pre
          class="max-h-96 overflow-auto rounded border border-border-soft p-3">{{ JSON.stringify( detail.debug, null, 2 ) }}</pre>
      </section>
        ...
        <template v-for="week in weeks" :key="`week-${week.weekNumber}`">
          <section v-if=" selectedWeekNumber === week.weekNumber ">
            <div class="mb-3 flex flex-wrap items-center justify-between gap-2">
              ...
              <span v-if="week.sourceMenuName" class="rounded-full bg-accent/10 dark:/30 px-3 py-1 text-xs dark:">
                {{ week.sourceMenuName }}
              </span>
            </div>
...
=======
              class="rounded-lg border bg-surface-1 p-4 text-text-1">
>>>>>>> Stashed changes
              <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                ...
                <span v-if=" day.score != null " class="rounded-full px-2 py-1 text-xs font-semibold" :class="day.meets_targets ? ' text-success' : ' text-warning' ">
...
=======
                <article v-for=" meal in day.meals " :key=" meal.id " class="rounded-lg border p-3 bg-surface-1"
>>>>>>> Stashed changes
                  :class="meal.is_special ? '' : ''">
                  <div class="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      <p class="text-xs font-semibold uppercase text-text-3">
                        {{ mealLabel( meal.meal_type ) }}
                        <span v-if=" Number( meal.meal_slot || 1 ) > 1 ">
                          · Plato {{ meal.meal_slot }}
                        </span>
                      </p>
                      <h4 class="font-semibold text-text-1">{{ meal.dish_name }}</h4>
                      <p v-if=" meal.dish_description " class="text-xs text-text-3">
                        {{ meal.dish_description }}
                      </p>
                    </div>
                    <span v-if=" meal.is_special " class="rounded-full bg-surface-3 px-2 py-1 text-xs text-text-2">
                      Comida libre · {{ meal.special_kcal_reserved ?? 700 }} kcal reservadas
                    </span>
                  </div>

                  <div v-if=" meal.is_special "
                    class="mt-3 rounded border border-border-soft bg-surface-2/50 p-3 text-xs text-text-3">
...
=======
                <div class="overflow-x-auto rounded-lg border bg-surface-1">
                  <table class="min-w-[820px] w-full text-sm text-text-1">
                    <thead class="text-left text-text-2">
>>>>>>> Stashed changes
                      <tr>
...
=======
      <section class="rounded-lg border bg-surface-1 p-4 text-text-1">
        <h2 class="font-semibold text-text-1">Lista de la compra generada</h2>
...
>>>>>>> Stashed changes
        ...
        <div v-if=" detail.shopping_items.length === 0 "
          class="mt-3 rounded border border-dashed p-4 text-sm text-text-3">
        ...
        <div v-else class="mt-3 grid gap-2 md:grid-cols-2">
          <div v-for=" item in detail.shopping_items " :key=" item.id " class="rounded border p-3 text-sm">
            <p class="font-medium text-text-1">{{ item.item_name }}</p>
            <p class="text-xs text-text-3">
