...
export default defineEventHandler(async (event) => {
  ...
  if (!body.userId || !body.phoneNumber) {
    throw createError({ statusCode: 400, statusMessage: 'userId y phoneNumber son obligatorios.' })
  }
  ...
  if (!config.twilioAccountSid || !config.twilioAuthToken) {
    await markSendStatus(supabase, body.userId, 'error', 'Twilio no está configurado en el servidor.')
    throw createError({ statusCode: 503, statusMessage: 'Twilio no está configurado en el servidor.' })
  }
  ...
  if (!from) {
    await markSendStatus(supabase, body.userId, 'error', 'Falta el número emisor.')
    throw createError({ statusCode: 503, statusMessage: 'Falta el número emisor.' })
  }

  const to = channel === 'whatsapp' && !body.phoneNumber.startsWith('whatsapp:')
    ? `whatsapp:${body.phoneNumber}`
    : body.phoneNumber
  const fromAddress = channel === 'whatsapp' && !from.startsWith('whatsapp:')
    ? `whatsapp:${from}`
    : from
...
async function markSendStatus(supabase: any, userId: string, status: string, error: string | null) {
  await supabase
    .from('shopping_lists')
