<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      ...
      <p v-if="statusMessage" class="text-sm" :class="statusType === 'error' ? 'text-red-600' : 'text-green-600'">
        {{ statusMessage }}
      </p>
    </div>
    ...
    <div class="grid gap-6 lg:grid-cols-[1fr_1.2fr]">
      <section class="bg-white rounded-lg shadow-sm border p-6">
        ...
        <div class="space-y-4">
          <label class="block">
            <span class="block text-sm font-medium text-gray-700 mb-1">Calorías diarias</span>
            <input
          ...
          <div class="rounded-lg border p-4" :class="macroValidation.valid ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'">
            ...
            <p class="text-xs mt-2" :class="macroValidation.valid ? 'text-green-700' : 'text-red-700'">
              {{ macroValidation.valid ? macroSummaryText : macroValidation.message }}
            </p>
          </div>
          ...
          >
            <div>
              ...
              <p class="text-sm text-gray-500">
                {{ sexLabel(profile.sex) }} · {{ profile.age }} años · {{ profile.daily_kcal_target }} kcal
              </p>
            </div>
            <div class="flex gap-2">
              <button @click="editProfile(profile)" class="px-3 py-1.5 text-sm text-indigo-600 hover:bg-indigo-50 rounded">
                Editar
              </button>
              <button @click="deleteProfile(profile.id)" class="px-3 py-1.5 text-sm text-red-600 hover:bg-red-50 rounded">
                Eliminar
              </button>
            </div>
          </div>
...
import { macroTargetsFromCalories, validateMacroTargets } from '~/utils/nutrition.js'
...
const saveConfig = async () => {
  ...
  saving.value = true
  const proteinTarget = macroTargetsFromCalories(config.value.daily_kcal_target, {
    fatPct: config.value.fat_pct_target,
    carbsPct: config.value.carbs_pct_target,
  }).protein_g
