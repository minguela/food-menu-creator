<template>
  <div class="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <section class="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
        ...
        <article class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
          ...
          <div class="mt-4 flex flex-wrap gap-2">
            <button class="rounded-lg bg-indigo-600 px-4 py-2 text-white hover:bg-indigo-700 disabled:opacity-50"
            ...
            <button class="rounded-lg border px-4 py-2 text-gray-700 hover:bg-gray-50 disabled:opacity-50"
        ...
        <article class="rounded-lg border bg-white p-4">
          ...
          <div v-if=" currentJob " class="mt-4 rounded-lg border p-3 text-sm" :class=" currentJob.status === 'failed'
          ...
          <div v-if=" currentJob " class="mt-4 rounded-lg border bg-zinc-950 p-3 text-sm text-zinc-100">
            ...
            <ol v-else class="max-h-80 space-y-2 overflow-y-auto pr-1">
              <li v-for=" log in generationLogs " :key=" log.id "
                ...
                <details v-if=" hasLogMetadata( log ) " class="mt-2 text-[11px] text-zinc-400">
                  <summary class="cursor-pointer select-none">metadata</summary>
                  <pre
                    class="mt-2 overflow-x-auto rounded border border-zinc-800 p-2">{{ JSON.stringify( log.metadata, null, 2 ) }}</pre>
                </details>
              </li>
            </ol>
          </div>
      ...
      <section v-if=" profilesSummary.length > 0 " class="rounded-lg border bg-white p-4">
      ...
      <section v-if=" generatedDays.length > 0 " class="space-y-4">
        <article v-for=" day in generatedDays " :key=" day.day_number " class="rounded-lg border bg-white p-4">
